...
Next, we want to make sure we can differentiate between movie vs. series. This is because we  aren interested in generes, and those may be dependent on wether it is a movie vs. series. But we also want to see if we can solve some missing values and drop some irrelevant columns about attributes. 
...
We are interest in genre patterns over time. However, these patterns may change not as quickly as on a yearly basis. Therefore, we construct a window that captures 5 years of viewer patterns. Because we do not know how these averRating votes we dynamic over time (for series), we assume that the yearStart is the year in which the most number of votes (numVotes) are generated. This is logical, because if this is positive the series will be continued, and if the reveiws are negative the series will be liekly to be discontinued. How the rating drops as the perc quality of the series drops over time, is for the orupose of this study left out of the equation.
...
Let's see how the actual ratings are then distrubuted across genres
```{r}
...
Linearity looks chaotics but okay. It may be a little smaller on the left and increases to the right but this is very little. So last, linearity. This is tricky, because there is no reason for genre to lead to a higher or lower rating. This is really specific to the watcher and also how the movie/series are doing. But when we add the time variable it can be that there is a relation between these elements, albeit unlikely to be linear. 
