...
movie_data$numVotes_group = cut(movie_data$numVotes, breaks = c(-Inf, 100, Inf), labels = c("Below 100", "Above 100"))
print(table(movie_data$numVotes_group))

below_100 = movie_data %>% mutate
  select(numVotes) %>%
  filter(numVotes < 100)

below_100

movie_data = movie_data %>% filter(numVotes_group == 'Above 100')
