...
```{r include=FALSE}
mdc <- read_csv("../data_prep/movie_data_cleaned.csv")
```
...
```{r echo=FALSE}
coeftable(simple_model)
```

```{r echo=FALSE}
coeftable(fixed_type_model)
```
...
Next we want to see how a specific genre evolves over the years. 

```{r}
list_of_coeficients <- coef(fixed_tth_model) %>%
  select(-id, -sample.var)

list_of_standard_deviations <- se(fixed_tth_model) %>%
  select(-id,-sample.var)

list_of_coeficients_long <- list_of_coeficients %>%
  pivot_longer(cols = -sample, names_to = "genre", values_to = "coef_value") %>%

# Pivot standard deviations longer
list_of_standard_deviations_long <- list_of_standard_deviations %>%
  pivot_longer(cols = -sample, names_to = "genre", values_to = "std_dev_value")
  
# Calculate p_threshold for each genre
graph_data <- left_join(list_of_coeficients_long, list_of_standard_deviations_long, by = "sample") %>%
  mutate(across(c(genre.x, genre.y), as.factor)) %>% 
  filter(genre.x == genre.y) %>%
  mutate(p_threshold = coef_value / std_dev_value,
         p_significance = ifelse(p_threshold > 1.96, 1, 0))
...
graph_data <- graph_data %>%
  mutate(
    lower_ci = coef_value - z_value * std_dev_value,
    upper_ci = coef_value + z_value * std_dev_value
  )
...
ggplot(graph_data, aes(x = sample, y = coef_value, color = factor(p_significance))) +
  ...
  geom_errorbar(aes(ymin = lower_ci, ymax = upper_ci), width = 0.2) +
  scale_color_manual(values = c("red", "green")) +
  xlab("Estimates") + ylab("Years") + facet_wrap(~genre.x) +
  scale_x_discrete(breaks = unique(graph_data$sample)[seq(1, length(unique(graph_data$sample)), length.out = 4)])
```

As you can see some genres are never significant, e.g., sports and thriller. While others become significant and never stop, e.g., history. There are also speaks in genres like war, where it seems to be falling in and out of favor. 
