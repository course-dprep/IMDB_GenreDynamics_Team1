...
# Create a directory and path to save the plots in
dir.create("gen/data_prep/output/figure", recursive = T)
path <- "../../gen/data_prep/output/figure/"
```
...
ggsave(filename = "Distribution_averageRating_int1.pdf", plot = last_plot(), device = "pdf", path = path)
```
...
ggplot(movie_data, aes(x = averageRating)) +
  ...
  labs(title = "Distribution of averageRating",
       x = "averageRating", y = "Frequency") +
  theme_minimal()

ggsave(filename = "Distribution_averageRating_int0.5.pdf", plot = last_plot(), device = "pdf", path = path)
```
...
ggsave(filename = "Distribution_numVotes.pdf", plot = last_plot(), device = "pdf", path = path)
...
ggsave(filename = "Distribution_numVotes_below100.pdf", plot = last_plot(), device = "pdf", path = path)
```
...
Good. But before we can continue we have to do some 'eye-econometric' and show if there might be data pattern before we run our analysis. First, we look at how number of votes and rating behave for each genre.
...
ggsave(filename = "averageRating_x_numVotes_by_genre.pdf", plot = last_plot(), device = "pdf", path = path)
```
...
ggsave(filename = "averageRating_frequency_by_genre.pdf", plot = last_plot(), device = "pdf", path = path)
```
...
ggsave(filename = "averageRating_qq_plot.pdf", plot = last_plot(), device = "pdf", path = path)
```
...
ggsave(filename = "averageRating_homoscedasticity.pdf", plot = last_plot(), device = "pdf", path = path)
```
...
ggsave(filename = "averageRating_independence.pdf", plot = last_plot(), device = "pdf", path = path)
```
...
ggsave(filename = "averageRating_linearity.pdf", plot = last_plot(), device = "pdf", path = path)
```
